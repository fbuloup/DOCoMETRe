...
public class IOParsers {
  ...
  /**
   * creates a Delaunay Triangulation from all the points in the suggested tsin file or from a smf
   * file (off like). if the file name is .smf - read it as an smf file as try to read it as .tsin
   * <br>
   * Note: duplicated points are ignored! <br>
   * SMF file has an OFF like format (a face (f) is presented by the indexes of its points -
   * starting from 1 - not from 0): <br>
   * begin <br>
   * v x1 y1 z1 <br>
   * ... <br>
   * v xn yn zn <br>
  ...
  private static List<Point> readSmf(BufferedReader is) throws IOException {
    ...
    while (s != null && s.charAt(0) == 'v') {
      ...
      st.nextToken();
      double d1 = Double.valueOf(st.nextToken()).doubleValue() * dx + minX;
      double d2 = Double.valueOf(st.nextToken()).doubleValue() * dy + minY;
      double d3 = Double.valueOf(st.nextToken()).doubleValue() * dz + minZ;
  ...
  private static List<Point> readTsin(BufferedReader is, String firstLine) throws IOException {
    ...
    while ((s = is.readLine()) != null) {
      ...
      double d3 = Double.valueOf(st.nextToken()).doubleValue();
      points.add(new Point(d1, d2, d3));
    }
    return points;
  }
