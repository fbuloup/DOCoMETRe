...
public class Point implements Comparable<Point>, Serializable {
  ...
  /**
   * tests the relation between this point (as a 2D [x,y] point) and a 2D segment a,b (the Z values
   * are ignored), returns one of the following: LEFT, RIGHT, INFRONTOFA, BEHINDB, ONSEGMENT
   * 
  ...
  /*
   * public ajSegment Bisector( ajPoint b) { double sx = (x+b.x)/2; double sy = (y+b.y)/2; double dx
   * = b.x-x; double dy = b.y-y; ajPoint p1 = new ajPoint(sx-dy,sy+dx); ajPoint p2 = new
   * ajPoint(sx+dy,sy-dx); return new ajSegment( p1,p2 ); }
   */
  ...
  Point circumcenter(Point a, Point b) {
    ...
    return new Point((u * (b.y - y) - v * (a.y - b.y)) / den,
  ...
  /**
   * return true iff this point [x,y] coordinates are the same as p [x,y] coordinates. (the z value
   * is ignored).
   */
  @Override
