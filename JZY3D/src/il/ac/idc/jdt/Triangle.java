...
public class Triangle implements Serializable {
  ...
  public Triangle(Point A, Point B, Point C) {
    ...
    if ((res <= Point.LEFT) || (res == Point.INFRONTOFA) || (res == Point.BEHINDB)) {
      ...
      System.out
          .println("Warning, ajTriangle(A,B,C) " + "expects points in counterclockwise order.");
  ...
  public BoundingBox getBoundingBox() {
    ...
    lowerLeft = new Point(Math.min(a.getX(), Math.min(b.getX(), c.getX())),
        Math.min(a.getY(), Math.min(b.getY(), c.getY())));
    upperRight = new Point(Math.max(a.getX(), Math.max(b.getX(), c.getX())),
  ...
  Circle circumcircle() {
    ...
    double u = ((a.getX() - b.getX()) * (a.getX() + b.getX())
        + (a.getY() - b.getY()) * (a.getY() + b.getY())) / 2.0f;
    double v = ((b.getX() - c.getX()) * (b.getX() + c.getX())
        + (b.getY() - c.getY()) * (b.getY() + c.getY())) / 2.0f;
    double den = (a.getX() - b.getX()) * (b.getY() - c.getY())
        - (b.getX() - c.getX()) * (a.getY() - b.getY());
    if (den == 0) // oops, degenerate case
      circum = new Circle(a, Double.POSITIVE_INFINITY);
    else {
      Point cen = new Point((u * (b.getY() - c.getY()) - v * (a.getY() - b.getY())) / den,
   ...
   * @return true iff p is not null and is inside this triangle (Note: on boundary is considered
   *         inside!!).
   */
  ...
  public double zValue(Point q) {
    if (q == null || this.halfplane)
      throw new RuntimeException(
          "*** ERR wrong parameters, can't approximate the z value ..***: " + q);
  ...
  /**
   * compute the Z value for the X,Y values of q. assume this triangle represent a plane --> q does
   * NOT need to be contained in this triangle.
   * 
