...
public class CameraDistanceAnnotation extends Point {
	...
	public void draw(IPainter painter) {
		...
		for (Drawable drawable : graph.getDecomposition()) {
			...
			// drawPolygonPointsDistanceToCamera(painter, h, v, screenOffset, colorPt,
			// strat, drawable);
		}
	}

	private void drawBarycenterDistanceToCamera(IPainter painter, Horizontal h, Vertical v, Coord2d screenOffset,
			Color colorBary, AbstractOrderingStrategy strat, Drawable drawable) {
		...
		txt.drawText(painter, Font.Helvetica_12, Utils.num2str(d, 4), drawable.getBarycentre(), h, v, colorBary,
				screenOffset);
	}

	@SuppressWarnings("unused")
	private void drawPolygonPointsDistanceToCamera(IPainter painter, Horizontal h, Vertical v, Coord2d screenOffset,
			Color colorPt, AbstractOrderingStrategy strat, Drawable drawable) {
		double d;
		if (drawable instanceof Geometry) {
			...
			for (Point pt : p.getPoints()) {
				...
				txt.drawText(painter, Font.Helvetica_12, Utils.num2str(d, 4), pt.getCoord(), h, v, colorPt,
						screenOffset);
			}
		}
	}
