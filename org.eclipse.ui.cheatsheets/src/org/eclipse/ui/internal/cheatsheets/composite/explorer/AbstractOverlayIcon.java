...
public abstract class AbstractOverlayIcon extends CompositeImageDescriptor {
	...
	protected void drawBottomLeft(ImageDescriptor[] overlays) {
		...
		for (int i = 0; i < 3; i++) {
			if (i < length && overlays[i] != null) {
				CachedImageDataProvider cachedImageDataProvider = createCachedImageDataProvider(overlays[i]);
				drawImage(cachedImageDataProvider, x, getSize().y - cachedImageDataProvider.getHeight());
				x += cachedImageDataProvider.getWidth();
			}
		}
	}
	...
	protected void drawCompositeImage(int width, int height) {
		ImageData imageData = getBaseImageData();
		Image image = new Image(PlatformUI.getWorkbench().getDisplay(), imageData);
		CachedImageDataProvider cachedImageDataProvider = createCachedImageDataProvider(image);
		drawImage(cachedImageDataProvider, 0, 0);
		if (fOverlays != null) {
	...
	protected void drawTopLeft(ImageDescriptor[] overlays) {
		...
		for (int i = 0; i < 3; i++) {
			if (i < length && overlays[i] != null) {
				CachedImageDataProvider cachedImageDataProvider = createCachedImageDataProvider(overlays[i]);
				drawImage(cachedImageDataProvider, x, 0);
				x += cachedImageDataProvider.getWidth();
			}
		}
	}
