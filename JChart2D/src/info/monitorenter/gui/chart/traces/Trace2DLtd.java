...
public class Trace2DLtd extends ATrace2D implements ITrace2D {
	...
	private boolean showCurrentTraceValue;
	 ...
	 * <p>
	 * 
	 * @param maxsize the buffer size for the maximum amount of points that will be
	 *                shown.
	 */
	...
	public boolean isShowCurrentTraceValue() {
		return showCurrentTraceValue;
	}

	public void setShowCurrentTraceValue(boolean showCurrentTraceValue) {
		this.showCurrentTraceValue = showCurrentTraceValue;
	}

	@Override
	public final String getName() {
		String name = super.getName();
		if(showCurrentTraceValue && !m_buffer.isEmpty() && !"".equals(name)) {
			double value = m_buffer.getYoungest().getY();
	 ...
	 * <p>
	 * 
	 * Don't use this too often as decreases in size may cause expensive array copy
	 * operations and new searches on all points for bound changes.
	 * <p>
	 * 
	...
	public final void setMaxSize(final int amount) {
		...
		synchronized (this.m_renderer) {
			...
			synchronized (this) {
				...
				this.minXSearch();
				if (this.m_minX != xmin) {
					this.firePropertyChange(ITrace2D.PROPERTY_MIN_X, Double.valueOf(xmin), Double.valueOf(this.m_minX));
				}

				final double xmax = this.m_maxX;
				this.maxXSearch();
				if (this.m_maxX != xmax) {
					this.firePropertyChange(ITrace2D.PROPERTY_MAX_X, Double.valueOf(xmax), Double.valueOf(this.m_maxX));
				}

				final double ymax = this.m_maxY;
				this.maxYSearch();
				if (this.m_maxY != ymax) {
					this.firePropertyChange(ITrace2D.PROPERTY_MAX_Y, Double.valueOf(ymax), Double.valueOf(this.m_maxY));
				}

				final double ymin = this.m_minY;
				this.minYSearch();
				if (this.m_minY != ymin) {
					this.firePropertyChange(ITrace2D.PROPERTY_MIN_Y, Double.valueOf(ymin), Double.valueOf(this.m_minY));
				}
			}
		}
	}
