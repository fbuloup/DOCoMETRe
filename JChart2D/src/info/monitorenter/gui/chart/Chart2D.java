 ...
 * 
 * <p>
 * <h3>PropertyChangeEvents</h3> {@link java.beans.PropertyChangeListener}
 ...
 * <td>if a new axis is added in x dimension
 * ({@link #addAxisXBottom(AAxis)}, @link {@link #addAxisXTop(AAxis)}).</td>
 * </tr>
 * <tr>
 * <td>{@link #PROPERTY_AXIS_X}</td>
 * <td>{@link Chart2D}</td>
 ...
 * <td>{@link ITrace2D}</td>
 * <td>{@link ITrace2D}</td>
 * <td>If a change of the traces occurs. If the old value is null a new trace
 * has been added. If the new value is null, oldvalue trace has been removed. If
...
public class Chart2D extends JPanel implements PropertyChangeListener, Iterable<ITrace2D>, Printable {
	...
	public static enum PointFinder implements IPointFinder {
		 ...
		 * <p>
		 * This implementation is slower than MANHATTAN but has a search field in form
		 * of a circle which is more natural for human eyes.
		 * 
		 ...
		 * Uses the Manhattan distance to find the nearest point.
		 * <p>
		 * This implementation is faster than EUCLID (only subtractions/additions/abs)
		 * but has a search field in form of a rhombus which may confuse human eyes.
		 * 
	...
	final class PointHighlighter extends MouseMotionAdapter
			implements MouseMotionListener, PropertyChangeListener, Serializable {
		...
		/**
		 * Default constructor, adds property change listener for point highlighters to
		 * the enclosing chart.
		 * <p>
		 */
		public PointHighlighter() {
			/*
			 * Need an identity hash map as the trace keys will change upon points added
			 * thus making them "unfindable" in a map based on hashcode!
			 */
		...
		/**
		 * Attaches the highlighters of the trace of the point to the point: it will be
		 * highlighted then.
		 * <p>
		 * 
		...
		private void clearOutdatedHighlighters(final ITrace2D trace) {
			...
			if (previousHighlightedPoint != null) {
				Iterator<IPointPainter<?>> itAdditionaPainters = previousHighlightedPoint.getAdditionalPointPainters()
						.iterator();
				...
				while (itAdditionaPainters.hasNext()) {
					...
					for (IPointPainter<?> highlighter : highlighters) {
						/*
						 * Cannot rely on "contains" method as comparable/equals might judge an equal
						 * highlighter added externally via api as a highlighter
						 */
		 ...
		 * <p>
		 * 
		 * TODO: Peek at getToolTipText invocations: Save operations in case the mouse
		 * has not moved much?
		 * 
		 ...
		 * <code>{@link ITrace2D#removeAllPointHighlighters()}</code>) are changed in
		 * order to re-attach the proper point highlighters to the highlighted points.
		 * <p>
		 * Also a hook is installed for
		 * <code>{@link Chart2D#PROPERTY_ADD_REMOVE_TRACE}</code> to clear out a
		 * reference to a highlighted trace to make it garbage-collectable in case the
		 * trace is removed.
		 * <p>
		 * 
		...
		public void propertyChange(PropertyChangeEvent evt) {
			...
			if (ITrace2D.PROPERTY_TRACEPOINT_CHANGED_HIGHLIGHTERS.equals(property)) {
				...
				throw new RuntimeException("Programming error: " + this.getClass().getName()
						+ " only has to be registered to the event " + ITrace2D.PROPERTY_TRACEPOINT_CHANGED_HIGHLIGHTERS
		 ...
		 * <p>
		 * 
		 * @param onoff if true, highlighting will be enabled, if false it will be
		 *              deactivated.
		 * 
	...
	public static enum ToolTipType implements IToolTipType {
		 ...
		 * <p>
		 * Note that this implementation only works correctly for one left y axis and
		 * one bottom x axis as it does not search for the nearest trace. Displayed
		 * values will be formatted according to the formatting of the axes mentioned
		 * above.
		 * <p>
		 */
		DATAVALUES {
		...
		/**
		 * Snaps to the nearest <code>{@link ITracePoint2D}</code> and shows it's value.
		 * <p>
	...
	/**
	 * A switch for debugging problems with highlighting. Set to false the compiler
	...
	/**
	 * A switch for debugging problems with scaling. Set to false the compiler will
	...
	/**
	 * The bean property <code>constant</code> identifying a change of the minor
	 * grid stroke.
	 * <p>
	 * Use this constant to register a {@link java.beans.PropertyChangeListener}
	 * with the <code>Chart2D</code>.
	...
	/**
	 * The bean property <code>constant</code> identifying a change of the major
	 * grid stroke.
	 * <p>
	 * Use this constant to register a {@link java.beans.PropertyChangeListener}
	 * with the <code>Chart2D</code>.
	...
	/**
	 * The bean property <code>constant</code> identifying a change of the internal
	 * <code>{@link IPointFinder}</code> instance used to find the nearest point
	 * corresponding to mouse events over the chart.
	 * <p>
	 * Use this constant to register a {@link java.beans.PropertyChangeListener}
	 * with the <code>Chart2D</code>.
	 * <p>
	...
	/**
	 * The bean property <code>constant</code> identifying a change of the tool tip
	 * type (<code>{Chart2D{@link #setToolTipType(IToolTipType)}</code>). <br/>
	 * <p>
	 * Use this constant to register a {@link java.beans.PropertyChangeListener}
	 * with the <code>Chart2D</code>.
	 * <p>
	 ...
	 * <p>
	 * The first element is always existing and is the downward compatible result of
	 * the call <code>{@link Chart2D#getAxisX()}</code>.
	 * <p>
	 */
	 ...
	 * <p>
	 * The first element is always existing and is the downward compatible result of
	 * the call <code>{@link Chart2D#getAxisY()}</code>.
	 * <p>
	 */
	...
	/**
	 * Flag to remember whether this chart has synchronized it's x start coordinates
	 * with another chart.
	 */
	 ...
	 * <p>
	 * The given axis must not be contained before (e.g. as right y axis or bottom x
	 * axis...).
	 * <p>
	 * 
	 ...
	 * <p>
	 * 
	 * @param listener the point highlighter listener to add.
	 */
	 ...
	 * <p>
	 * This method will trigger a {@link java.beans.PropertyChangeEvent} being fired
	 * on all instances registered by
	 * {@link javax.swing.JComponent#addPropertyChangeListener(java.lang.String, java.beans.PropertyChangeListener)}
	...
	/**
	 * Adds the trace to this chart with relation to the given x axis and y axis. It
	 * will be painted (if it's {@link ITrace2D#isVisible()} returns true) in this
	 * chart.
	 * <p>
	 * This method will trigger a {@link java.beans.PropertyChangeEvent} being fired
	 * on all instances registered by
	 ...
	 * 
	 * @param points the trace to add.
	 * @param xAxis  the x axis responsible for the scale of this trace - it has to
	 *               be contained in this chart or an exception will be thrown.
	 * @param yAxis  the y axis responsible for the scale of this trace - it has to
	 *               be contained in this chart or an exception will be thrown.
	 * 
	 * @see IAxis#PROPERTY_ADD_REMOVE_TRACE
	 */
	public final void addTrace(final ITrace2D points, final IAxis<?> xAxis, final IAxis<?> yAxis) {
		...
		/*
		 * 
		 * This lock is needed to ensure a lock on the tree is acquired before a lock to
		 * the chart. Method paint will also first acquire the treeLock and then has to
		 * get the lock on the chart while this code path might descend into ChartPanel
		 * where Container.getComponents() (in method containsTraceLabel()) will require
		 * the tree lock after having already acquired the chart lock -> deadlock
		 * between paint thread and addTrace thread.
		 */
		if (Chart2D.DEBUG_THREADING) {
		...
		synchronized (this.getTreeLock()) {
			/*
			 * synchronization necessary to avoid that a highlighter is added to the trace
			 * while counting them.
			 */
			if (Chart2D.DEBUG_THREADING) {
			...
			synchronized (this) {
				if (Chart2D.DEBUG_THREADING) {
					System.out
							.println(Thread.currentThread().getName() + ", addTrace(ITrace2D, XAxis, YAxis): 2 locks.");
				}
				synchronized (points) {
					if (Chart2D.DEBUG_THREADING) {
						System.out.println(
	 ...
	 * paint the x labels and the maximum amount of characters that are used for the
	 * x labels (<code>{@link IAxisLabelFormatter#getMaxAmountChars()}</code>)
	 * because an x label may occur on the right edge of the chart and should not be
	 * clipped.
	 * <p>
	 * 
	...
	private int calculateXChartEnd(final Graphics g) {
		...
		if (this.m_axesYRight.size() > 0) {
			...
			/*
			 * Use the maximum label width of the x axes to avoid x labels being clipped in
			 * case there are no right y axes:
			 */
	 ...
	 * <p>
	 * This value depends on the current <code>{@link FontMetrics}</code> used to
	 * paint the y labels and the maximum amount of characters that are used for the
	 * y labels (<code>{@link IAxisLabelFormatter#getMaxAmountChars()}</code> ).
	 * <p>
	 * 
	 ...
	 * <p>
	 * The value computed here is the maximum overhang of all y axes caused by their
	 * font height of their labels or the summation of all top x axis heights (if
	 * this is greater) .
	 * <p>
	 * 
	...
	private int calculateYChartEnd(final Graphics2D g2d) {
		 ...
		 * 
		 * TODO: maybe this is too much work in case all fonts of all axes are the same
		 * and every axis will give the same result for the height (debug).
		 */
	...
	public JToolTip createToolTip() {
		 ...
		 * setTipText into a View (with BasicHTML) and sets it as the putClientProperty
		 * BasicHtml.html of itself.
		 */
	 ...
	 * adds and removes charts. So if you use the same Chart2D object(s) during the
	 * applications lifetime there is no need to use this method.
	 * <p>
	 */
	public void destroy() {
		if (Chart2D.DEBUG_THREADING) {
	 ...
	 * 
	 * @param onoff if true the closest point to the cursor will be highlighted, if
	 *              false you might gain performance by having this feature turned
	 *              off!
	 * 
	 * @return true if a change of this state did take place (you did not call this
	 *         at least twice with the same argument).
	 */
	...
	private void ensureUniqueAxis(final IAxis<?> axisToAdd) {
		if (this.m_axesXBottom.contains(axisToAdd)) {
			throw new IllegalArgumentException(
					"Given axis (" + axisToAdd.getAxisTitle().getTitle() + " is already configured as bottom x axis!");
		}
		if (this.m_axesXTop.contains(axisToAdd)) {
			throw new IllegalArgumentException(
					"Given axis (" + axisToAdd.getAxisTitle().getTitle() + " is already configured as top x axis!");
		}
		if (this.m_axesYLeft.contains(axisToAdd)) {
			throw new IllegalArgumentException(
					"Given axis (" + axisToAdd.getAxisTitle().getTitle() + " is already configured as left y axis!");
		}
		if (this.m_axesYRight.contains(axisToAdd)) {
			throw new IllegalArgumentException(
					"Given axis (" + axisToAdd.getAxisTitle().getTitle() + " is already configured as right y axis!");
		}
	...
	/**
	 * Returns the x axis that the given trace belongs to or null if this trace does
	 * not belong to any x axis of this chart.
	 * <p>
	 * 
	 * @param trace the trace to find the corresponding x axis of this chart for.
	 * 
	 * @return the x axis that the given trace belongs to or null if this trace does
	 *         not belong to any x axis of this chart.
	 */
	...
	/**
	 * Returns the y axis that the given trace belongs to or null if this trace does
	 * not belong to any y axis of this chart.
	 * <p>
	 * 
	 * @param trace the trace to find the corresponding y axis of this chart for.
	 * @return the y axis that the given trace belongs to or null if this trace does
	 *         not belong to any y axis of this chart.
	 */
	...
	/**
	 * Returns the chart - wide setting for the ms to give a repaint operation time
	 * for collecting several repaint requests into one (performance vs. update
	 * speed).
	 * <p>
	 * 
	 ...
	 * <p>
	 * This method is expensive and should not be used when rendering fast changing
	 * charts with many points.
	 * <p>
	 ...
	 * square root, addition and quadrature for Euclid distance). However the euclid
	 * distance spans a circle for the nearest points which is visually more normal
	 * for end users than the Manhattan distance which forms a rhombus and reaches
	 * far distances in only one dimension.
	 * <p>
	 * 
	 ...
	 * 
	 * @TODO: This is called twice per mouse move (tool tip and highlighter): Cache
	...
	/**
	 * Returns the chart that will be synchronized for finding the start coordinate
	 * of this chart to draw in x dimension (<code>
	 * {@link #getXChartStart()}</code> ).
	 * <p>
	 * This feature is used to allow two separate charts to be painted stacked in y
	 * dimension (one below the other) that have different x start coordinates
	 * (because of different y labels that shift that value) with an equal starting
	 * x value (thus be comparable visually if their x values match).
	 * <p>
	 * 
	 * @return the chart that will be synchronized for finding the start coordinate
	 *         of this chart to draw in x dimension (<code>
	 *         {@link #getXChartStart()}</code>).
	 */
	...
	public final String getToolTipText(final MouseEvent event) {
		 ...
		 * ToolTipType.VALUE_SNAP_TO_TRACEPOINTS! This is done for 2 reasons: 1. Don't
		 * compute the point related to a mouse event twice. 2. Don't allow highlighted
		 * points with a tool tip that shows the data of the coordinates (vs. the data
		 * of the point highlighted) which might be misleading!
		 */
		return result;
	}
	 ...
	 * like <code>{@link Set#add(Object)}</code> or <code>{@link Set#clear()}</code>
	 ...
	 * have been interpolated to a bound of the chart to avoid drawing horizontal or
	 * vertical lines at the bound. In case two points that were not interpolated
	 * really have a vertical or horizontal connection the result would not be quite
	 * correct in terms of painting: Their line would not be drawn even if it would
	 * be correct.
	 * <p>
	 * 
	 ...
	 * means that two points interpolated to the maximum and the minimum (x or y) do
	 * not have to be tested for intersections with the viewport rectangle.
	 * <p>
	 * 
	 ...
	 * <p>
	 * 
	 * This is only necessary for the charts whose left y axes should start more to
	 * the right than if they were all on their own and didn't have to care for
	 * other charts.
	 * <p>
	 * 
	 * @param g2d needed for size information.
	 ...
	 * <p>
	 * 
	 * <h4>Contract</h4> The new axis already has to be added to a chart to avoid a
	 * NPE when adding the traces of the old one!
	 * <p>
	 * 
	...
	private void internalTransferAxisState(IAxis<?> old, AAxis<?> axisNew) {
		...
		/*
		 * add the traces: this has to be after adding axis to avoid npe in addTrace as
		 * no chart is set!
		 */
		for (ITrace2D trace : traces) {
	...
	public boolean isVisible(final ITracePoint2D point) {
		...
		if (point == null) {
			...
			result = !(Double.isNaN(point.getScaledX()) || Double.isNaN(point.getScaledY()))
					&& !(point.getScaledX() > 1.0 || point.getScaledX() < 0.0 || point.getScaledY() > 1.0
	...
	private void listenToTrace(final ITrace2D trace) {
		...
		if (trace instanceof ITrace2DDataAccumulating) {
			...
			trace.addPropertyChangeListener(
					ITrace2DDataAccumulating.PROPERTY_ACCUMULATION_STRATEGY_ACCUMULATION_FUNCTION_CHANGED, this);
		}
	}
	 ...
	 * <p>
	 * 
	 * @param ref the highlighted trace point.
	 */
	 ...
	 * done in that order in <code>{@link #addTrace(ITrace2D, IAxis, IAxis)}</code>.
	 * <p>
	 * 
	 ...
	 * acquired the chart lock -> deadlock between paint thread and addTrace thread
	 ...
	 * details.
	 * <p>
	 * If you do invoke this method you may encounter performance issues, flickering
	 * UI and even deadlocks.
	 * <p>
	 * 
	...
	protected synchronized void paintComponent(final Graphics g) {
		...
		while (traceIt.hasNext()) {
			...
			if (trace.isVisible()) {
				synchronized (trace) {
					...
					while (pointIt.hasNext()) {
						...
						countPoints++;
						/*
						 * Special case: if we have NaN just don't interpolate anything or paint but
						 * just continue (and give a signal to trace painters to discontinue which is
						 * neccessary for implementations that paint polylines and must not accumulate
						 * polylines that have a discontinuation within):
						 */
						...
						if (isNaNNewpoint || isNanOldpoint) {
							/*
							 * Only discontinue when entering NaN space as calls to it for subsequent NaN
							 * values would repeat the same polyline paint of the last valid point in
							 * TracePainterPolyline (senseless).
							 */
							...
							int tmptmpy = tmpy;
							/*
							 * check if the interconnection of both invisible points cuts the visible area:
							 */
							...
							if (this.hasChartIntersection(oldpoint, newpoint)) {
								...
								this.paintPoint(oldtmpx, oldtmpy, tmpx, tmpy, true, trace, g, oldpoint, false, rangex,
										rangey);
							}
			...
			if (Chart2D.DEBUG_THREADING) {
				System.out.println(
						"paint(" + Thread.currentThread().getName() + "), left lock on trace " + trace.getName());
			}
			if (this.firstPaint) {
				this.firstPaint = false;
	 ...
	 * overview. This method may only be called by {@link #paint(Graphics)} and the
	 * order of this invocation there must not be changed.
	 * <p>
	 * 
	 ...
	 * <p>
	 * The current point to render in px is defined by the first two arguments, the
	 * next point to render in px is defined by the 2nd two arguments.
	 * <p>
	 * 
	 * @param trace       needed to get the {@link IErrorBarPolicy} instances to
	 *                    use.
	 * @param oldtmpx     the x coordinate of the original point to render an error
	 *                    bar for.
	 * @param oldtmpy     the y coordinate of the original point to render an error
	 *                    bar for.
	 * @param tmpx        the x coordinate of the original next point to render an
	 *                    error bar for.
	 * @param tmpy        the y coordinate of the original next point to render an
	 *                    error bar for.
	 * @param g2d         the graphics context to use.
	 * @param discontinue if a discontinuity has been taken place and all potential
	 *                    cached points by an
	 *                    <code>{@link ITracePainter}</code> (done for polyline
	 *                    performance boost) have to be drawn immediately before
	 *                    starting a new point caching.
	 * @param original    intended for information only, should nor be needed to
	 *                    paint the point neither be changed in any way!
	 */
	private void paintErrorBars(final ITrace2D trace, final int oldtmpx, final int oldtmpy, final int tmpx,
			final int tmpy, final Graphics g2d, final boolean discontinue, final ITracePoint2D original) {
		IErrorBarPolicy<?> errorBarPolicy;
	 ...
	 * {@link ITracePainter}) and error bar painter ({@link IErrorBarPolicy}) of the
	 * trace.
	 * <p>
	 * This method must not be called directly as it does not support interpolation
	 * of visibility bounds (discontinuations).
	 * <p>
	 * 
	 * @param xPxOld          the x coordinate of the previous point to render in px
	 *                        (potentially an interpolation of it if the old point
	 *                        was not visible and the new point is).
	 * 
	 * @param yPxOld          the y coordinate of the previous point to render in px
	 *                        (potentially an interpolation of it if the old point
	 *                        was not visible and the new point is).
	 * 
	 * @param xPxNew          the x coordinate of the point to render in px
	 *                        (potentially an interpolation of it if the old point
	 *                        was visible and the new point is not).
	 * 
	 * @param yPxNew          the y coordinate of the point to render in px
	 *                        (potentially an interpolation of it if the old point
	 *                        was visible and the new point is not).
	 * 
	 * @param trace           needed for obtaining trace painters and error bar
	 *                        painters.
	 * 
	 * @param g2d             the graphics context to use.
	 * 
	 * @param discontinue     if a discontinuation has been taken place and all
	 *                        potential cached points by an
	 *                        <code>{@link ITracePainter}</code> (done for polyline
	 *                        performance boost) have to be drawn immediately before
	 *                        starting a new point caching.
	 * 
	 ...
	 * 
	 * @param rangeX          needed in case the highlighters highlight not the
	 *                        exact coordinate of the point.
	 * 
	 * @param rangey          needed in case the highlighters highlight not the
	 *                        exact coordinate of the point.
	 */
	private final void paintPoint(final int xPxOld, final int yPxOld, final int xPxNew, final int yPxNew,
			final boolean discontinue, final ITrace2D trace, final Graphics g2d, final ITracePoint2D original,
			final boolean errorBarSupport, final double rangeX, final double rangeY) {
		Iterator<ITracePainter<?>> itTracePainters;
		ITracePainter<?> tracePainter;
		itTracePainters = trace.getTracePainters().iterator();
		while (itTracePainters.hasNext()) {
	...
	/**
	 * Receives all <code>{@link PropertyChangeEvent}</code> from all instances the
	 * chart registers itself as a <code>{@link PropertyChangeListener}</code> .
	 * <p>
	 * 
	...
	public void propertyChange(final PropertyChangeEvent evt) {
		...
		synchronized (this) {
			...
			if (property.equals(IRangePolicy.PROPERTY_RANGE)) {
				...
				/*
				 * TODO: perhaps react more fine grained for the following events: just repaint
				 * the trace without all the paint code (scaling, axis,...). But: These property
				 * changes are triggered by humans and occur very seldom. Huge work non-l&f
				...
				/*
				 * Relay the event as outsiders don't want to deal with internals (listen to
				 * axes to be informed whenever a trace was added). Also: repaint definetely!
				 */
				...
				System.out.println("Received an unexpected property change event \"" + property
						+ "\" the code is not expecting (programming error). Event: " + evt);
			}
			this.setRequestedRepaint(true);
		}
		if (Chart2D.DEBUG_THREADING) {
	 ...
	 * <p>
	 * This method is broken down to every axis contained in the trace and will fire
	 * a <code>{@link PropertyChangeEvent}</code> for the
	...
	public Set<ITrace2D> removeAllTraces() {
		...
		/*
		 * We skip "result.addAll(...) for y axes as by contract every trace has to be
		 * at least in one x axis (not logical if trace is e.g. in y axes only).
		 */
		// 2.1) axes y left:
		it = this.m_axesYLeft.iterator();
		while (it.hasNext()) {
	 ...
	 *         not configured as a bottom x axis before or could not be removed for
	 *         another reason.
	 */
	 ...
	 * 
	 * @return a copied List with the previous bottom x
	 *         <code>{@link IAxis}</code> instance that was used at position 0.
	 */
	@Deprecated
	...
	/**
	 * Sets the bottom x axis on the given position to use and replaces the old one
	 * on that place.
	 * <p>
	 * This method delegates to <code>{@link #addAxisXBottom(AAxis)}</code> and also
	 * uses <code>{@link #removeAxisXBottom(IAxis)}</code> in case a bottom x axis
	 * was configured at the position. So the events
	 ...
	 * unregistering from the old axis / registering to the new axis from outside to
	 * give them the chance to manage their state transitions by themselves.<br/>
	 ...
	 * with <code>{@link Chart2D#PROPERTY_AXIS_X_BOTTOM_REPLACE}</code> as code and
	 * the old and new axis as values and have the change to change their peer to
	 * listen on thus receiving the change events generated on the new axis. At the
	 * moment the replace event is sent they will already have received the event
	 * <code>{@link Chart2D#PROPERTY_AXIS_X}</code> event for the removal: So be
	 * careful not to react on that first event by removing your listener from the
	 * axis as you then will not receive the replace event!
	 * <p>
	 * 
	 * @param axisX    the first bottom x axis to use.
	 * 
	 * @param position the index of the axis on bottom x dimension (starting from
	 *                 0).
	 * 
	...
	/**
	 * Sets the top x axis on the given position to use and replaces the old one on
	 * that place.
	 * <p>
	 * This method delegates to <code>{@link #addAxisXBottom(AAxis)}</code> and also
	 * uses <code>{@link #removeAxisXBottom(IAxis)}</code> in case a top x axis was
	 ...
	 * 
	 * Furthermore this method uses "replace - semantics". The
	 * <code>{@link ITrace2D}</code> instances contained in the previous x top axis
	 * will be implanted to this new axis. Also the title and stuff like grid
	 * settings will be transferred. For this an event with property
	 ...
	 * Before the state of the old axis is transferred they will receive
	 * <code>{@link PropertyChangeListener#propertyChange(PropertyChangeEvent)}</code>
	 * with <code>{@link Chart2D#PROPERTY_AXIS_X_TOP_REPLACE}</code> as code and the
	 * old and new axis as values and have the change to change their peer to listen
	 * on thus receiving the change events generated on the new axis. At the moment
	 * the replace event is sent they will already have received the event
	 * <code>{@link Chart2D#PROPERTY_AXIS_X}</code> event for the removal: So be
	 * careful not to react on that first event by removing your listener from the
	 * axis as you then will not receive the replace event!
	 * <p>
	 ...
	 * 
	 * @return a copied List with the previous left y
	 *         <code>{@link AAxis}</code> instance that was used at position 0.
	 */
	@Deprecated
	...
	/**
	 * Sets the left y axis on the given position to use and replaces the old one on
	 * that place.
	 * <p>
	 ...
	 * 
	 * Furthermore this method uses "replace - semantics". The
	 * <code>{@link ITrace2D}</code> instances contained in the previous left y axis
	 * will be implanted to this new axis. Also the title and stuff like grid
	 * settings will be transferred. For this an event with property
	 * <code>{@link Chart2D#PROPERTY_AXIS_Y_LEFT_REPLACE} </code> is fired.
	 * <p>
	 ...
	 * listen on thus receiving the change events generated on the new axis. At the
	 * moment the replace event is sent they will already have received the event
	 * <code>{@link Chart2D#PROPERTY_AXIS_Y}</code> event for the removal: So be
	 * careful not to react on that first event by removing your listener from the
	 * axis as you then will not receive the replace event!
	 * <p>
	 * 
	...
	public IAxis<?> setAxisYLeft(final AAxis<?> axisY, final int position) {
		 ...
		 * confused. Add anyways in case no axis has been set before (see constructor)
		 */

		this.addAxisYLeft(axisY);
		/*
		 * We can only transfer state (which includes adding traces after the new axis
		 * is assigned to a chart!
		 */
		if (old != null) {
	 ...
	 * on that place.
	 * <p>
	 * This method delegates to <code>{@link #addAxisYRight(AAxis)}</code> and also
	 * uses <code>{@link #removeAxisYRight(IAxis)}</code> in case an axis was
	 * configured on the position. So the events
	 * <code>{@link Chart2D#PROPERTY_AXIS_Y}</code> will be fired for remove and
	 * add.
	 * <p>
	 ...
	 * <p>
	 * 
	 * @param gridStroke the major grid stroke to use or null if the feature should
	 *                   be turned off.
	 */
	 ...
	 * <p>
	 * 
	 * @param gridStroke the minor grid stroke to use or null if the feature should
	 *                   be turned off.
	 */
	 ...
	 * <p>
	 * 
	 * @param minPaintLatency the setting for the ms to give a repaint operation
	 *                        time for collecting several repaint requests into one
	...
	public void setPaintLabels(final boolean paintLabels) {
		...
		if (change) {
			this.firePropertyChange(Chart2D.PROPERTY_PAINTLABELS, Boolean.valueOf(!paintLabels),
	...
	/**
	 * Sets the point finder used to find the nearest point corresponding to a mouse
	 * event.
	 * <p>
	 * 
	 ...
	 * 
	 * @param pointFinder the point finder used to find the nearest point
	 ...
	 * implementation does not guarantee to collapse several repaint requests into a
	 * single one but prefers to issue many paint invocations causing a high CPU
	 * load in realtime scenarios (adding several 100 points per second to a chart).
	 * <p>
	...
	/**
	 * Sets the chart that will be synchronized for finding the start coordinate of
	 * this chart to draw in x dimension ( <code>{@link #getXChartStart()}
	 * </code>).
	 * <p>
	 * This feature is used to allow two separate charts to be painted stacked in y
	 * dimension (one below the other) that have different x start coordinates
	 ...
	 * <p>
	 * 
	 * @param synchronizedXStartChart the chart that will be synchronized for
	 *                                finding the start coordinate of this chart to
	 ...
	 * <p>
	 * 
	 * @param toolTipType one of the available
	 *                    <code>{@link ToolTipType}</code> constants.
	 * 
	 * @see Chart2D.ToolTipType#DATAVALUES
	 * @see Chart2D.ToolTipType#NONE
	 * @see Chart2D.ToolTipType#PIXEL
	...
	/**
	 * Returns a BufferedImage with the current width and height of the chart filled
	 * with the Chart2D's graphics that may be written to a file or OutputStream by
	 * using:
	 ...
	 * was not integrated into layout correctly or the chart's dimenision was set to
	 * this value, a default of width 600 and height 400 will temporarily be set
	 * (syncrhonized), the image will be rendered, the old dimension will be reset
	 * and the image will be returned.<br/>
	 * If you want to paint offscreen images (without displayed chart) prefer invoke
	 * {@link #snapShot(int, int)} instead.
	 * <p>
	 * 
	 ...
	 * <p>
	 * 
	 * @param addedOrRemovedTraceHighlighters the amount of added or removed
	 *                                        (negative) trace highlighters.
	 */
	 ...
	 * related to the axes assigned to the trace that owns the nearest point to the
	 * given mouse event's coordinates.
	 * <p>
	 ...
	 *         event to the value within the chart or null if no calculations could
	 *         be performed as the chart was not painted before / had no
	 *         data/traces.
	 * 
	 * @throws IllegalArgumentException if the given mouse event does not belong to
	 *                                  this component.
	 */
	 ...
	 * <p>
	 * 
	 * @param force if true no detection of changes of the data bounds as described
		...
		while (it.hasNext()) {
			...
			if (changed) {
				...
				if (Chart2D.DEBUG_SCALING) {
					System.out.println(
							"updateScaling: Scaling was performend for axis: " + currentAxis.getAxisTitle().getTitle());
				}
			} else {
				if (Chart2D.DEBUG_SCALING) {
