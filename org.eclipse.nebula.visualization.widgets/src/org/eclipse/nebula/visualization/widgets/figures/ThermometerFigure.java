...
public class ThermometerFigure extends AbstractLinearMarkedFigure {	
			...
			if(effect3D && support3D){
				...
				Pattern backPattern = null;
				 ...
				 backPattern = GraphicsUtil.createScaledPattern(graphics, Display.getCurrent(), 
					bounds.x + getLineWidth(), bounds.y + getLineWidth(), 
					bounds.x+bounds.width-getLineWidth(), bounds.y+bounds.height-getLineWidth(),
	...
	class Pipe extends RoundedRectangle {		
		...
		@Override
		protected void fillShape(Graphics graphics) {
			getCornerDimensions().height =PIPE_WIDTH/2;
			getCornerDimensions().width = PIPE_WIDTH/2;
		...
		public Dimension getPreferredSize(int wHint, int hHint) {
			return new Dimension(PIPE_WIDTH , hHint+2*getCornerDimensions().height); 
		}
